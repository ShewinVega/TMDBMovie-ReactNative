...
const MovieDetail = () => {
  ...
  return (
    <View className="bg-primary flex-1 pb-20">
          ...
          <View className="mt-5">
            ...
            <View className="flex-row items-start gap-2 mt-2">
              {movie?.genres.map((genre) => (
                <View
                  key={genre.id}
                  className="flex-row items-center justify-center bg-light-300 px-2 py-1 rounded-md"
                >
      ...
      <TouchableOpacity
        className="absolute left-0 right-0 bottom-0 mx-5 bg-accent rounded-lg py-3.5  flex flex-row items-center justify-center z-50"
        onPress={() => router.back()}
      >
        <Image
          source={icons.arrow}
